...
@objc @IBDesignable class CometChatMessageHeader: UIView , NibLoadable {
    ...
    func onTextMessageReceived(textMessage: TextMessage) {
        ...
        print("Header: \(textMessage.stringValue())")
    ...
    @IBAction func didInfoButtonPressed(_ sender: Any) {
        if let currentUser = currentUser {
            CometChatMessageHeader.messageHeaderDelegate?.onInfoClick(user: currentUser)
        }
        if let currentGroup = currentGroup {
            CometChatMessageHeader.messageHeaderDelegate?.onInfoClick(group: currentGroup)
        }
    }
}
