...
class CometChatPollsBubble: UIView {
    ...
    private func commonInit() {
        CometChatUIKit.main.loadNibNamed("CometChatPollsBubble", owner: self, options: nil)
...
extension CometChatPollsBubble: UITableViewDelegate, UITableViewDataSource {
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        cell.callBack = { [weak self] in
            ...
            Indicator.show()
            CometChat.callExtension(slug: "polls", type: .post, endPoint: "v2/vote", body: ["vote": indexPath.row + 1, "id": strongSelf.pollID]) { response in
                Indicator.hide()
            } onError: { error in
