...
class CometChatWhiteboardBubble: UIView {
    ...
    @IBAction func onOpenWhiteboardClick() {
        ...
        if let controller = controller , let customMessage = customMessage {
            ...
            if let metaData = customMessage.metaData , let injected = metaData["@injected"] as? [String : Any], let cometChatExtension =  injected["extensions"] as? [String : Any], let collaborativeDictionary = cometChatExtension["whiteboard"] as? [String : Any], let collaborativeURL =  collaborativeDictionary["board_url"] as? String {
