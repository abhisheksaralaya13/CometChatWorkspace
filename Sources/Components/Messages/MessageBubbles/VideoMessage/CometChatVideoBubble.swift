...
class CometChatVideoBubble: UIView {
    ...
    private func parseThumbnailForImage(message: MediaMessage) {
        ...
        if let metaData = message.metaData , let injected = metaData["@injected"] as? [String : Any], let cometChatExtension =  injected["extensions"] as? [String : Any], let dict = cometChatExtension["thumbnail-generation"] as? [String : Any] {
