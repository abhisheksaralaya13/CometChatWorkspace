...
//            }
    ...
    private func commonInit() {
        Bundle.module.loadNibNamed("CometChatMemberList", owner: self, options: nil)
            ...
            if let groupMember = selectedCell.groupMember {
                CometChat.banGroupMember(UID: groupMember.uid ?? "", GUID: self.group?.guid ?? "") { bannedSuccess in
                    ...
                    if let group = self.group {
                        CometChatGroups.emitOnGroupMemberBan(bannedUser: groupMember, bannedGroup: group)
                    }
                    DispatchQueue.main.async {
